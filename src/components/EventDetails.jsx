  ...
  return (
    <div className="max-w-4xl mx-auto mt-12 p-6 bg-gray-900 text-white rounded-2xl shadow-lg space-y-6">
      {loading ? (
        ...
        <>
          <h2 className="text-3xl font-bold text-indigo-400 text-center mb-6">Event Details</h2>

          <div className="space-y-2 text-gray-300">
            <p><span className="font-semibold text-indigo-300">Titel:</span> {event.title}</p>
            <p><span className="font-semibold text-indigo-300">Datum:</span> {new Date(event.date).toLocaleString("de-DE")}</p>
            <p><span className="font-semibold text-indigo-300">Beschreibung:</span> {event.description}</p>
            <p><span className="font-semibold text-indigo-300">Ort:</span> {event.location}</p>
            <p><span className="font-semibold text-indigo-300">Breite (S/N):</span> {event.latitude}</p>
